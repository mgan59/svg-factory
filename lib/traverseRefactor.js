...
var traverseNode = function(svgDoc, srcJS, parentGroup, group, depth){
    ...
    for(var ctr=0; ctr < svgDoc.length; ctr++){
        ...
        if(_.has(svgDoc[ctr], 'children')){
            ...
            // check `name` attr which will determine how we traverse
            // name = `svg` topRootNode
            if(node.name === 'svg'){
                // first pass so we should define out parentGroup
                //return traverseNode(node.children, srcJS, parentGroup, '', _depth);
                // top level element needs to iterate over each child and
                // call the traverse
                var test = traverseNode(node.children, srcJS, parentGroup, '', _depth);
                console.log('top svg stack end?');
            ...
            // name = `g` a group element
                ...
                var test = traverseNode(node.children, srcJS, parentGroup, groupID, _depth);
